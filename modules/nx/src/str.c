...
static bool my_get_str(ks_str_builder sb, void* data, nx_dtype dtype, int N, nx_size_t* dim, nx_size_t* stride, int depth) {
    ...
    if (N == 0) {
        ...
        #define GET_STR_LOOP(NXT_DTYPE_0, NXT_TYPE_0) for (i = 0; i < dim0; ++i, dptr_A += stride[0]) { \
            ...
            if (NXT_DTYPE_0->kind == NX_DTYPE_KIND_CINT) ks_str_builder_add_fmt(sb, "%z", (ks_ssize_t)*(NXT_TYPE_0*)dptr_A); \
            else if (NXT_DTYPE_0->kind == NX_DTYPE_KIND_CFLOAT) { \
                int len = snprintf(tmp, sizeof(tmp) - 1, "%5.2f", (double)*(NXT_TYPE_0*)dptr_A); \
                ks_str_builder_add(sb, tmp, len); \
            } \
                ...
                ks_throw(ks_T_TodoError, "Internal dtype '%S' not handled in get_str", NXT_DTYPE_0); \
                return false; \
            } \
        }
        ...
        if (dim[0] >= trunc_size) {
            ks_str_builder_add_fmt(sb, " ...(%z) ", dim[0] - trunc_size);
        ...
        for (i = 0; i < dim0; i++, data_i += stride[0]) {
            if (i > 0) ks_str_builder_add_fmt(sb, "\n%*c", depth + 1, ' ');
