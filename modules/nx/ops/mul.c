...
static bool my_mul_1d(int N, nxar_t* arrs, int len, void* _user_data) {
    NX_ASSERT_CHECK(N == 3);
...
// calc A * B -> C
bool nx_T_mul(nxar_t A, nxar_t B, nxar_t C) {

    // apply the ufunc
    return nx_T_ufunc_apply(3, (nxar_t[]){ A, B, C }, my_mul_1d, NULL);
}
