...
static ks_obj cvtNum(ks_obj obj, int minTY) {
    ...
    if (obj->type != ks_T_int && obj->type != ks_T_float && obj->type != ks_T_complex) {
        return NULL;
    }
    if (minTY == TY_INT) {
...
#define _TY_ERR(_a, _types) { \
    return ks_throw(ks_T_TypeError, "Could not convert '%T' object to a valid type: %s", _a, _types); \
}
...
#define T_M_F_2f(_name, _cfunc_f, ...) static KS_TFUNC(m, _name) { \
    ks_obj __tmp0, __tmp1; \
    KS_GETARGS("arg0 arg1", &__tmp0, &__tmp1); \
    if (args[0]->type == ks_T_complex) _TY_ERR(args[0], "float"); \
    if (args[1]->type == ks_T_complex) _TY_ERR(args[1], "float"); \
    ks_obj a0 = cvtNum(args[0], TY_FLOAT); \
    if (!a0) _TY_ERR(args[0], "float"); \
...
#define T_M_F_2fc(_name, _cfunc_f, _cfunc_c, ...) static KS_TFUNC(m, _name) { \
    ...
    { __VA_ARGS__; }; \
    if (a0->type == ks_T_float && a1->type == ks_T_float) { \
        ks_obj ret = (ks_obj)ks_float_new(_cfunc_f(((ks_float)a0)->val, ((ks_float)a1)->val)); \
        KS_DECREF(a0); KS_DECREF(a1); \
        return ret; \
    } else { \
...
// round(val) -> round a value
T_M_F_1fc(abs, fabs, cabs, if (a0->type == ks_T_complex) { ks_obj ret = (ks_obj)ks_float_new(cabs(((ks_complex)a0)->val)); KS_DECREF(a0); return ret; });
...
T_M_F_1fc(sqrt, sqrt, csqrt, if (a0->type == ks_T_float && ((ks_float)a0)->val < 0.0) return arg_error("val", "val >= 0");)
...
T_M_F_2fc(pow, pow, cpow, 
    if (
        a0->type == ks_T_float && a1->type == ks_T_float && 
        ((ks_float)a0)->val < 0 && floor(((ks_float)a1)->val) != ((ks_float)a1)->val
    ) { 
        return ks_throw(ks_T_MathError, "Cannot raise negative float to fractional float power (cast one or both to 'complex')"); 
    });
...
T_M_F_1fc(asin, asin, casin, if (a0->type == ks_T_float && fabs(((ks_float)a0)->val) > 1) return arg_error("val", "|val| <= 1");)
// acos(val) -> return the inverse cos of a given result
T_M_F_1fc(acos, acos, cacos, if (a0->type == ks_T_float && fabs(((ks_float)a0)->val) > 1) return arg_error("val", "|val| <= 1");)
...
T_M_F_1fc(acosh, acosh, cacosh, if (a0->type == ks_T_float && ((ks_float)a0)->val < 1) return arg_error("val", "val >= 1");)
// atanh(val) -> return the inverse tan of a given value
T_M_F_1fc(atanh, atanh, catanh, if (a0->type == ks_T_float && fabs(((ks_float)a0)->val) > 1) return arg_error("val", "|val| <= 1");)

// logarithm
// Use an optional 'base' argument which defaults to 'E' (natural logarithm)
static KS_TFUNC(m, log) {
    ks_obj __tmp, _base;
    KS_GETARGS("arg0 ?base", &__tmp, &_base);
    ...
    if (n_args == 2) {
        ...
        if (base->type == ks_T_complex || val->type == ks_T_complex) {
            // 1 or the other is complex
            double complex base_c, val_c;
            if (base->type == ks_T_complex) {
                base_c = ((ks_complex)base)->val;
                if (val->type == ks_T_complex) {
                    val_c = ((ks_complex)val)->val;
                } else {
                    val_c = ((ks_float)val)->val;
                }
            } else if (base->type == ks_T_float) {
        ...
        // natural logarithm
        if (val->type == ks_T_float) {
            ks_obj ret = (ks_obj)ks_float_new(log(((ks_float)val)->val));
            KS_DECREF(val);
            return ret;
        } else if (val->type == ks_T_complex) {
...
static complex double my_cgamma(complex double z) {
    if (creal(z) < 0.5) {
        ...
        return KS_M_PI / (csin(KS_M_PI * z) * my_cgamma(1 - z));
    } else {
        // shift off
        z -= 1;
...
T_M_F_1fc(gamma, tgamma, my_cgamma, if (a0->type == ks_T_float && ((ks_float)a0)->val <= 0 && floor(((ks_float)a0)->val) == ((ks_float)a0)->val) { return arg_error("x", "!(x <= 0 && is_int(x))"); });
...
double my_zeta(double x) {
    if (x < 0) {
        ...
        return 2 * pow(2 * KS_M_PI, x - 1) * sin(x * KS_M_PI / 2) * tgamma(1 - x) * my_zeta(1 - x);
...
double complex my_czeta(complex double x) {
    ...
    if (cr < 0) {
        ...
        // use reflection formula with the functional equation
        // Zeta(x) = 2 * (2*PI)^(x-1) * sin(x * PI/2) * Gamma(1-x) * Zeta(1 - x)
        return 2 * cpow(2 * KS_M_PI, x - 1) * csin(x * KS_M_PI / 2) * my_cgamma(1 - x) * my_czeta(1 - x);
