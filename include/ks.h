 ...
 * |-   Future Progress   -|
 * 
 * Currently, the biggest things to implement are:
 * 
 *   * Fully featured standard libraries; string functions, common operation shortcuts, and more need to be finished
 *   * Tests, checks, and etc
 *   * Tuple/iterable unpacking similar to Python (i.e. `a, b = func()`)
 * 
 * 
...
/* generated configuration header; if this is not found, you need to run `./configure` */
#include <ks-config.h>
...
#if defined(KS__WINDOWS) || defined(KS__CYGWIN) || defined(KS__MINGW)
//
#else
//
#endif
...
// ks_obj - the most generic kscript object, which any other objects are castable to
// This contains a 'type' & a 'refcnt' variable (as in the KS_OBJ_BASE macro), which describe the type that the object is,
//   as well as a count of active references to the object. One should not modify these, and instead use the macros
//   KS_INCREF(obj) & KS_DECREF(obj) respectively
// Once the reference count drops to 0, that means the object is effectively a 'dangling pointer', and thus can
//   be freed.
// Objects should always stay as pointers, so they aren't type-erased. Dereferencing
//   an object of another type could be bad, so don't do it! Use -> operations
struct ks_obj {
    KS_OBJ_BASE
};
...
struct ks_type {
    ...
    // type.__new__(self, *args) -> construct a new object of a given type. This should normally take 0 arguments
    ...
    // Sometimes, this is implemented as a 'pfunc' (i.e. partial function) with the 0th argument filled in as the type;
    //   this is neccessary when dealing with type hierarchies
    //
    ...
    //
    // However, mutable types (i.e. most types) should have __new__() be callable with zero arguments to simply generate a new
    ...
    //   * if __init__ is not found, the code is effectively: 'x = type.__new__(a, b, c); x.__type__ = type'
    //
    ks_obj __new__;

    // type.__init__(self, *args) -> initialize an object (i.e. the second part of the constructor);
    // See above in comments above '__new__' for an explanation of which is chosen and when
    ks_obj __init__;
    ...
    // type.__blob__(self) -> convert an item to a blob
    ks_obj __blob__;
...
struct ks_tuple {
    ...
    // This is possible due to the fact that a tuple is immutable; so once created,
    //   the number of elements is constant; no need for re- or de-allocation
    // NOTE: A reference is held to each of 'elems[:]'
    ks_obj elems[0];
...
struct ks_list {
    ...
    // This pointer is allocated seperately (and may be NULL if len==0)
    // NOTE: A reference is held to each of 'elems[:]'
    ks_obj* elems;
...
// ks_int - type representing an integer value in kscript
// It can hold values of anywhere between -2^2^32-1 and +2^2^32 (guaranteed even on a 32 bit machine), using
//   the GMP library (or mini-gmp, included when GMP is not on the current system)
// To do this, it has a union between an `int64_t` (native C type) and an `mpz_t`, the GMP arbitrary precision
//   integer. To know which is used, you need to check `->isLong`.
// BUT, you should (almost) never do this! You should use the appropriate `ks_num_get_*` methods to convert numerics
//   of any kind! They will attempt to convert between enumerations, booleans, floats, ints, etc (or anything that
//   implements those protocols)
// See the documentation for those functions, i.e. `ks_num_get_i64()`
typedef struct ks_int {
    ...
    // if `isLong`, then the value is stored as an `mpz_t` (see GMP library), or else, it is stored
    //   as the 64 bit integer `v_i64`
    bool isLong;
...
// &KS_SMALL_INTS[val + KS_SMALL_INT_MAX] == 'val' as a n integer
...
// NOTE: Technically, there are different NANs; should those be allowed ?
//   I think so, but that might be best saved for the `libc` library
...
typedef struct {
    ...
    // human readable name (default: <cfunc @ ADDR>, but use `ks_cfunc_new2(0)` to provide a signature)
    ks_str name_hr;
...
// Additionally, sometimes it is useful to release the GIL while an IO operation, or
//   large computation finishes; use the `ks_GIL_unlock()` and `ks_GIL_lock()` functions
typedef struct {
    KS_OBJ_BASE
...
typedef struct {
    ...
    // List of threads spawned by this thread
    // NOTE: These should all be of type `ks_thread`
    ks_list sub_threads;
    ...
    // the stack of objects for the thread
    // This is what the bytecode objects operate on
    ks_list stk;
...
//   NULL-status of the result of this function, unless you know what you are doing!
...
// Append all enum values directly to the dict object
KS_API void ks_dict_add_enum_members(ks_dict self, ks_type enumtype);
...
// If there was a problem, return NULL and throw an error
// NOTE: Returns a new reference
KS_API ks_blob ks_iostream_readblob_n(ks_iostream self, ks_ssize_t sz);
...
// Sleep (i.e. yield the thread) for a given duration (in seconds).
...
// %O - ks_obj, prints stupid simple format `<'%T' obj @ %p>`, but is guaranteed
//                never to call any kscript functions
...
// Create a new blob from a list of data
// NOTE: Returns a new reference
KS_API ks_blob ks_blob_new(ks_size_t len, uint8_t* data);
